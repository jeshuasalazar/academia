...
async function chatOpenAICompat(proveedor, modelo, system, user, json) {
...
let _anthropic = null;
async function chatAnthropic(modelo, system, user) {
 ...
 */
async function chat(modeloKey, { system, user, json }) {
  const m = MODELOS[modeloKey];
  ...
  if (m.proveedor === 'anthropic') return chatAnthropic(m.modelo, system, user);
  return chatOpenAICompat(m.proveedor, m.modelo, system, user, json);
}
