...
async function guionizar(nivel, leccion) {
  const user =
    ...
    `Cada slide lleva un "imagen_prompt": una metáfora VISUAL concreta para ilustrar la idea de ese slide, ` +
    `EN INGLÉS, describiendo objetos/escena (no texto), p.ej. "a robot reading a giant open book, glowing neurons". ` +
    `No incluyas palabras ni letras en la imagen; el estilo de marca se agrega aparte.\n` +
    `Devuelve JSON EXACTO: { "voz": "descripción breve del tono narrado", "slides": [ ` +
    `{ "id": 1, "capa": "simple"|"serio"|"experto", "titulo": "..", "bullets": ["..", ".."], ` +
    `"imagen_prompt": "concrete visual metaphor in English, no text", ` +
    `"narracion": "texto hablado, 2-4 frases naturales" } ] }. Todo en español de México (salvo imagen_prompt).`;
...
async function producir(leccion, guion, opts) {
  ...
  // ¿Ilustraciones IA por escena? Activo solo si hay proveedor+key (no-fatal).
  const imgProv = opts.imgProveedor || IMG.proveedor;
  const usarImagen = Boolean(imgProv) && disponibleImagen(imgProv);
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    // Ilustración de fondo (NO-FATAL: si falla, el slide usa el degradado de marca).
    let fondo = null;
    if (usarImagen && slide.imagen_prompt) {
      try {
        if (opts.generarImagen) fondo = (await opts.generarImagen(slide.imagen_prompt)).buffer;
        else fondo = (await generarImagen(slide.imagen_prompt, { proveedor: imgProv })).buffer;
      } catch (e) { fondo = null; }
    }

    const buf = await renderSlide(slide, { tema: leccion.titulo, fondo });
  ...
  // Por cada slide: un mini-clip mp4 (imagen + su audio) con fundidos y, si está
  // activo, Ken Burns (zoom/paneo lento) para dar vida sin costo extra.
  const [outW, outH] = VIDEO.resolucion.split('x').map(Number);
  const clips = [];
  for (let i = 0; i < segmentos.length; i++) {
    const s = segmentos[i];
    const clip = path.join(dir, `clip-${String(i).padStart(2, '0')}.mp4`);
    const fade = Math.min(VIDEO.fade, s.duracion / 3);
    const frames = Math.max(1, Math.round(s.duracion * VIDEO.fps));

    let vf;
    if (VIDEO.kenBurns) {
      // Pre-escala grande (reduce jitter del zoompan) y alterna zoom-in/zoom-out
      // + leve paneo según la paridad de la escena. z sube/baja ~12% en el clip.
      const zoomIn = i % 2 === 0;
      const z = zoomIn
        ? `min(zoom+0.0009,1.12)`
        : `if(lte(zoom,1.0),1.12,max(zoom-0.0009,1.0))`;
      const x = i % 4 < 2 ? `iw/2-(iw/zoom/2)` : `(iw-iw/zoom)*on/${frames}`;
      const y = `ih/2-(ih/zoom/2)`;
      vf = `scale=${outW * 1.4}:${outH * 1.4},` +
        `zoompan=z='${z}':d=${frames}:x='${x}':y='${y}':s=${outW}x${outH}:fps=${VIDEO.fps},` +
        `fade=t=in:st=0:d=${fade},fade=t=out:st=${Math.max(0, s.duracion - fade)}:d=${fade}`;
    } else {
      vf = `scale=${outW}:${outH},fps=${VIDEO.fps},` +
        `fade=t=in:st=0:d=${fade},fade=t=out:st=${Math.max(0, s.duracion - fade)}:d=${fade}`;
    }
  ...
  try {
    ...
    if (VIDEO.bunnyLibraryId && process.env.BUNNY_STREAM_API_KEY) {
      try {
        ...
        // La duración medida viaja con el video para que la UI muestre el
        // tiempo REAL (video + lectura) en vez de un valor inventado (Fix dur).
        if (video) video.duracionSeg = Math.round(duracionTotal);
      } catch (e) {
