...
// ---- Duración real de una sesión (video + lectura del contenido didáctico) --
// El tiempo mostrado SIEMPRE debe coincidir con el contenido: se suma la
// duración medida del video (video.duracionSeg, la estampa el pipeline) más el
// tiempo de lectura de las 7 secciones (~180 palabras/min). Si no hay contenido
// ni video, respeta el duration_minutes de la lección como último recurso.
function contarPalabras(txt) {
  return String(txt || '').trim().split(/\s+/).filter(Boolean).length;
}
function duracionSesionMin(sc, fallbackMin) {
  const CAMPOS = ['promesa', 'idea_simple', 'explicacion_seria', 'precision_experta',
    'ejemplo_guiado', 'error_comun', 'pregunta_pensar', 'mini_ejercicio', 'resumen',
    'descripcion', 'resumen', 'notas'];
  let palabras = 0;
  CAMPOS.forEach((k) => { palabras += contarPalabras(sc[k]); });
  (sc.puntos_clave || []).forEach((p) => { palabras += contarPalabras(p); });
  (sc.comprobacion || sc.preguntas_pase || []).forEach((q) => {
    palabras += contarPalabras(q.q || q.pregunta);
    (q.opciones || []).forEach((o) => { palabras += contarPalabras(o); });
  });
  const lecturaSeg = palabras / 3; // ~180 palabras por minuto
  const videoSeg = (sc.video && Number(sc.video.duracionSeg)) || 0;
  if (!palabras && !videoSeg) return fallbackMin || 0;
  return Math.max(1, Math.round((lecturaSeg + videoSeg) / 60));
}
    ...
    (c.course_modules || []).forEach((m) => {
      (m.lessons || [])
        ...
        .forEach((l) => {
          ...
          const dur = duracionSesionMin(sc, l.duration_minutes || 0);
          totalMin += dur;
          sesiones.push({
            id: l.id,
            modulo: m.id,
...
function anotarCurso(curso, userPlan, progresoMap) {
  ...
  // Desbloqueo secuencial: una sesión sólo está disponible si la anterior ya se
  // completó (la primera siempre abierta, si el plan lo permite). Esto encadena
  // la ruta: terminas una clase → se abre la siguiente.
  let prevCompletada = true;
  const sesiones = curso.sesiones.map((s) => {
    const p = progresoMap[s.id];
    const completada = p ? p.completada : false;
    const pct = p ? p.pct : 0;
    let locked = status.locked, lockReason = status.lockReason, lockPlan = status.lockPlan;
    if (!locked && !prevCompletada) { locked = true; lockReason = 'secuencia'; lockPlan = null; }
    prevCompletada = completada;
    return {
      ...s,
      locked, lockReason, lockPlan,
      // Contrato con el front (alumno.js lee s.progreso.*). Se dejan también los
      // campos planos por compatibilidad con cualquier consumidor viejo.
      pct, completada,
      progreso: { pct, completada }
    };
  });
  return { ...curso, sesiones, profesor: PROFESOR_DEFAULT, empresa: null };
}
